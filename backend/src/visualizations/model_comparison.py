...
class ModelComparison:
    ...
    def get_production_model(self, model_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"⚠️ No production model found for {model_name}: {e}", exc_info=True)
        return None
    ...
    def get_candidate_model(self, run_id: str, model_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics = run.data.metrics
            logger.info(f"📦 Retrieved candidate model {model_name} from run {run_id}")
            return {
                "run_id": run_id,
                "metrics": metrics,
            ...
            logger.error(f"❌ Failed to retrieve candidate model {model_name}: {e}", exc_info=True)
            return {}
    ...
    def compare_metrics(
        ...
        """⚖️ Compare production and candidate model metrics"""
        ...
        for metric_name, direction in comparison_metrics:
            if metric_name not in prod_metrics or metric_name not in candidate_metrics:
                logger.warning(f"⚠️ Metric {metric_name} missing in one of the models")
                continue
        ...
        key_metrics = ["test_rmse", "test_mae"]
        ...
        logger.info(f"📊 Comparison complete. Should promote: {should_promote}")
    ...
    def promote_model(
        ...
        try:
            ...
            logger.info(f"✅ Model {model_name} v{model_version.version} promoted to production.")
            return True

        except Exception as e:
            logger.error(f"❌ Failed to promote model {model_name}: {e}", exc_info=True)
            return False
    ...
    def evaluate_and_promote(
        ...
        """🧪📈 Evaluate models and promote if improved"""
        results = {}
        ...
        for model_name in model_names:
            ...
            if prod_model is None:
                if force_first_deployment:
                    logger.info(f"🚀 First deployment for model {model_name}")
